...
import com.suvin.nepia.global.config.properties.AppEnv;
...
public class JacksonConfig {
    ...
    public JacksonConfig(AppEnv appEnv) {
        this.date = appEnv.format().date();
        this.dateTime = appEnv.format().dateTime();
    }
    ...
    public Jackson2ObjectMapperBuilderCustomizer objectMapperBuilder() {
        return builder -> builder
                ...
                .serializers(
                        ...
                        new LocalDateTimeSerializer(DateTimeFormatter.ofPattern(dateTime)))
                /* 클라이언트로부터 전달된 JSON 데이터의 역직렬화 설정 */
                .deserializers(
                        new LocalDateDeserializer(DateTimeFormatter.ofPattern(date)),
                        new LocalDateTimeDeserializer(DateTimeFormatter.ofPattern(dateTime)))
